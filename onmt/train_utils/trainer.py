...
class XETrainer(BaseTrainer):
    ...
    # def __init__(self, model, loss_function, train_data, valid_data, dicts, opt, bert_vectors, setup_optimizer=True):
    #     super().__init__(model, loss_function, train_data, valid_data, dicts, opt, bert_vectors)
    ...
    def eval(self, data):
        ...
        with torch.no_grad():
            for i in range(len(data)):
                ...
                # # lets have a look the fist and the last sentence
                # sent1_ids = src_ids.t()[0].cpu().numpy().tolist()
                # sent2_ids = src_ids.t()[-2].cpu().numpy().tolist()
                # print(sent2_ids)
                # src_dict_obj = self.dicts['src']
                # src_id2w_dict = src_dict_obj.idxToLabel
                # sent1 = [src_id2w_dict[num] for num in sent1_ids]
                # sent2 = [src_id2w_dict[num] for num in sent2_ids]
                # print(sent1)
                # print(sent2)
